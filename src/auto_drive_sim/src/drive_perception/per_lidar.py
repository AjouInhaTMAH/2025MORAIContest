...
class PerLidar:
    ...
    def view_obstacle_with_ROI(self,points):
        ...
        # 우측 ROI (오른쪽, x: 0.175~0.525, y: 0~1.0 → y=앞, x=오른쪽)
        rx1 = int(cx + self.length_y_rotary_min * scale)
        ry1 = int(cy + self.length_x_rotary * scale)
        rx2 = int(cx + self.length_y_rotary_max * scale)
        ry2 = cy
        cv2.rectangle(img, (rx1, ry1), (rx2, ry2), (0, 0, 255), 1)  # Magenta
