...
# avoid_lidar_camera_yolov5.py에서 네비게이션 노드의 토픽을 받아, 차선 추종 영역을 제어,
# 이유 : 차선의 노이즈가 많아 정확한 차선 추적이 어려웠음, 네비게이션처럼 왼쪽으로 가라 -> 왼쪽 ROI 차선만 선택후 따라감,
...
class Traffic_control:
    def __init__(self):
        ...
        ## CAR NAVIGATION CALLBACK
        # rospy.Publisher('/lane_mode', Int32, queue_size=1)
    ...
    def cam_lane_detection(self,msg):
        # amcl_pose에 의한 navigation신호를 받아, 추종해야되는 차선을 선택한다. 
        self.img = self.bridge.compressed_imgmsg_to_cv2(msg)
        ...
        try:
            # 차선 노이즈가 많은 영역에 대해, car_navigation을 이용하여,
            #  해당 영역에서는 차선 보는 부분을 left or right에만 집중하게 만듬,
            if self.lane_mode == 1:
                # 🔵 왼쪽 차선만 추적 (zone 1)
                if left_count > LEFT_MIN_PIXELS:
                    center_index = ((left_indices[0] + left_indices[-1]) // 2) + 160
                    print("🟦 zone 1: 왼쪽 차선 추종")
    ...
    def action(self):
        ...
        # 1️⃣-2 LIDAR 회피 모드 - 정적 장애물
            ...
            if not self.in_avoid_mode:
                print("회피 방향 결정 중...") # 차선에 따른 회피 단계가 avoid side ,로 처리,,
        ...
        # 2️⃣ 일반 주행 (lane follow) - 차선 인식 주행
            ...
            error = self.center_index - self.standard_line # <- 차선쪽에서 CENTER_INDEX부분이 나옴, 
