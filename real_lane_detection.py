...
class lane_dec:
    def __init__(self):
        ...
        self.init_color()
        self.init_sliding()
        self.lane_cotrol = real_lane_controller.lane_ctrl()
    ...
    def sliding_window_lane_calculation(self,x_current,y_current,prev_margin,x_prev,blocked_flag, binary_img, is_left_lane , color=(0,255,0)):
        ...
        if len(good_inds) > self.minpix:
            ...
            if (max_x - min_x) > 100:   # ← 임계값: 윈도우 폭이 100px 이상이면 건너뛰기
        ...
        #print(f"[WIN] width: {max_x - min_x}, mean_x: {mean_x:.1f}, prev: {x_prev}, x_current: {x_current}")
    ...
    def processing(self):
        while not rospy.is_shutdown():
            ...
            self.lane_cotrol.action(self.dataset)
